...
| 3 | **Coach: add Available Days + Hours** | Show coach's available work days and free hours in coach details. | 🟡 Medium (3–4h) | ⬜ טרם בוצע |
| 4 | **Coach: add Work Region** | Display geographic work area for the coach. | 🟢 Small (1–2h) | ⬜ טרם בוצע — `availability_area` קיים במודל כ-JSONB אבל לא מוצג ב-UI |
...
| 11 | **Client: fix document icon click** | Document icon on Clients page doesn't open anything — wire up navigation/dialog. | 🟢 Small (1–2h) | ⬜ טרם בוצע — אייקון קיים ב-`Clients.razor` אבל `ViewContracts` ריק |
...
| # | Request | Explanation | Effort | Status |
|---|---------|-------------|--------|--------|
| 1 | **Model: add fields to `coaches` table** | Phone, available_days, available_hours, work_region. Requires DB migration + model update. | 🟡 Medium (2–3h) | 🟡 חלקי — `phone` נוסף למודל ול-DB, `availability_area` JSONB קיים. `available_days`/`available_hours` חסרים |
...
|----------|-------------|-----------|---------|-----------|----------------------|
| UI-only changes | 26 | 10 | 1 | 15 | ~22–35h |
| Architecture/Backend changes | 14 | 3 | 3 | 8 | ~49–77h |
| **Total** | **40** | **13** | **4** | **23** | **~71–112h** |
...
- UI #9: ימים ושעות מועדפים — מיושם ב-`ContractDialog.razor` עם JSON schedule
...
- Backend #1: שדות מאמן — `phone` בוצע, `available_days`/`available_hours` חסרים
