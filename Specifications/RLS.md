...
-- 1. client_contracts - הסכמי החיוב והרווח מול בתי הספר/עמותות מוסתרים ברובם,
--    למעט חריג: ראה migrations/007_coach_read_own_contract_engagement.sql
--    שמאפשר למאמן לקרוא (SELECT) רק את שורות ההתקשרות המקושרות לשיבוצים שלו
--    (כדי להציג "שם ההתקשרות" נכון במסכי המאמן, במקום "כללי" קבוע).
